...
Item {
    ...
    function timeChanged() {
        ...
        minutes = getMinutes();
        seconds = getSeconds();
        dateText.text = Qt.formatDate(new Date(), "d");

    }
    property bool running: true
    Connections{
        target:running? main:null;
        onTimeUpdated:{analogclock.timeChanged()}
    }
     ...
     Rectangle{
         ...
         border.color: colour;
         border.width: 2;
         height: largeScreen? Theme.itemSizeExtraSmall/1.6:Theme.itemSizeExtraSmall/1.7;
         width: largeScreen? Theme.itemSizeExtraSmall/1.4: Theme.itemSizeExtraSmall/1.5;
         anchors{
             verticalCenter:face.verticalCenter;
             verticalCenterOffset: 0;
             right: face.right;
             rightMargin:Screen.sizeCategory== Screen.Large?Theme.paddingLarge*5: Theme.paddingLarge*4
         }
     }
